...
public class VideoUtil {
    ...
    //检查视频时间是否一致，自定义比较列表
    public Boolean check_video_time(List<String> source, List<String> target) {
        long souce_long = 0L;
        long target_long = 0L;
        for (String s : source) {
            String source_time = get_video_time(s);
            source_time = source_time.substring(0, source_time.lastIndexOf("."));
            String[] strings = {source_time.substring(0, 2), source_time.substring(3, 5), source_time.substring(6, 8)};
            souce_long += Integer.parseInt(strings[0]) * 60 * 60 + Integer.parseInt(strings[1]) * 60 + Integer.parseInt(strings[2]);
        }
        for (String s : target) {
            String target_time = get_video_time(s);
            target_time = target_time.substring(0, target_time.lastIndexOf("."));
            String[] strings = {target_time.substring(0, 2), target_time.substring(3, 5), target_time.substring(6, 8)};
            target_long += Integer.parseInt(strings[0]) * 60 * 60 + Integer.parseInt(strings[1]) * 60 + Integer.parseInt(strings[2]);
        }
        if (souce_long == 0 || target_long == 0) {
            return false;
        }
        if (souce_long == target_long) {
            return true;
        }
        return false;
    }
    ...
    //检查视频时间是否一致，自定义比较列表
    public Boolean check_video_size(List<String> source, List<String> target) {
        long souce_long = 0L;
        long target_long = 0L;
        for (String s : source) {
            File file = new File(s);
            souce_long += file.length();
        }
        for (String s : target) {
            File file = new File(s);
            target_long += file.length();
        }
        if (souce_long == 0 || target_long == 0) {
            return false;
        }
